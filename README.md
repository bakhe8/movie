...
| `docker/` | Dev: PostgreSQL (`ankane/pgvector`, also hosts the e2e suite's `moviedb_test` database — board C-17), Redis 7. Staging/prod: `Dockerfile` per service, `docker-compose.prod.yml` (file-based secrets, one-shot migrations), backup/restore scripts |
