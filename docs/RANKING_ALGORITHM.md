...
- Policy versions: `random-v2` (uniform draw of 3 from watched-unranked, excluding sets already completed while an unseen one exists, $\rho = 1/\binom{n}{3}$ over the pool actually drawn from) and `adaptive-v1`, **implemented 2026-09-04 behind the `triad-policy` experiment flag** (ADR-83, `TriadPolicyService`): score = Σ_d (per-dimension uncertainty × spread across the three titles) − repeat penalty − genre repetition, no two titles by one director (§8.3), sampled from the top-K with $\rho = 1/K$, also never a completed set (ADR-99). Uncertainty is the trainer's Laplace standard errors (ADR-62), flat before a posterior exists. Every triad row records its `policyVersion`; with no running experiment row every profile stays on the random policy.
- `purpose` and `countsTowardActivation` (ADR-99, revising ADR-34's one-triad lookback): a triad's *set* (its three title ids, independent of display order) is never repeated by either policy while the watched pool (minus the triad just completed) can still make an unseen one. Once it cannot, the round is still drawn -- `BP §8.1`'s "verification/refutation in an independent context" -- but labelled `purpose: 'verify'` and `countsTowardActivation: false`: it counts toward neither the training threshold (`TrainingService.countCompleted`) nor the trainer's query (`TRAINABLE_TRIAD_PREDICATE`), so ten rounds over the same three films can never read as ten pieces of evidence.
