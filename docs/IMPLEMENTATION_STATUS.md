...
Not fixed, left open: a `@typescript-eslint/eslint-plugin@^6.19.0` (backend devDependency) chain and a `vitest@^1.1.0` chain, both dev-tooling only, unrelated to NestJS — 10 vulnerabilities (1 critical, 7 high, 2 moderate) in `npm audit`'s full (non-`--omit=dev`) report. Production (`--omit=dev`) is clean.
...
**Last updated**: 2026-09-03 · **Status**: core loop (auth → mark watched → rank → train by CLI → recommend) runs locally; six blueprint gaps, five security/code-quality audit findings, and the NestJS 10→11 migration (ADR-29) closed today; `npm audit --omit=dev` clean; nine blueprint-conformance pieces still fall short (list above).
