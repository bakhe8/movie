...
**Still open**: `rankLibrary()` deliberately does not write here — it's a different surface by design (ADR-33), not an oversight. No `outcomes`/`watch_events` recording exists yet, so the post-watch loop (`§4.5`) still can't close end to end, and nothing has read this new log back for evaluation (`§16`) — this closes the write side of gap 4 only.
...
| Item | Built | Blueprint | Evidence / gap |
|---|---|---|---|
| `GET /profiles/:id/recommendations` (409 until a snapshot exists) | ✅ | 🟡 | `§13.1`/`§14`: now persists one row per shown result (reason, propensity, `requestId` all logged) since 2026-09-03; still gap 4's other half — no `outcomes` are ever recorded against it |
...
| Confidence band (verbal, no %) | ✅ | 🟡 | band from triad count, fingerprint-quality demotion, and now held-out prediction success (ADR-59); posterior stability and diversity still missing (gap 5) |
