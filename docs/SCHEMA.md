...
- **§1 Current physical schema** — exactly what the twenty-four TypeORM migrations in `apps/backend/src/migrations/` create (verified 2026-09-04). This is the truth for anyone writing SQL today.
- **§2 Target schema** — the `BP §13.1` entity set expressed as tables, plus the migration plan from §1 to §2.

Naming (ADR-16): tables `snake_case` plural; columns are TypeORM's default `camelCase` and therefore **quoted** in raw SQL (`"profileId"`); primary keys `uuid` via `uuid_generate_v4()`; timestamps `TIMESTAMP` (UTC by convention). The one plural-naming exception (`user_title_state`) was renamed to `user_title_states` in M1. Schema changes go through `npm run migration:generate` / `npm run db:migrate` only (in a built image there is no TypeScript and no `ts-node`, so the deployed migrate step is `npm run migrate:prod` → `node dist/migrate.js`, A-19) — `synchronize` is off in every environment.

---

## 1. Current physical schema (migrated)

POSTERS-MULTI / [ADR-120](ARCHITECTURE_DECISIONS.md#adr-120--additional-posters-are-rows-in-title_posters-not-a-second-column-on-titles-posters-multi-p1-2026-09-06): migration `1788494000000-AddTitlePosters` creates `title_posters`, one row per poster image a title has. Schema only — nothing writes to it yet (P2 backfills it from TMDB, P3 reads it). `titles.posterPath` is unchanged and stays the single poster every current read path uses.
...
password_resets (                                                -- ALPHA_PLAN 3.2 / ADR-85: single-use, time-limited reset link
  id uuid PK, "userId" uuid NOT NULL FK users ON DELETE CASCADE,
  "tokenHash" varchar(64) UNIQUE NOT NULL,                       -- sha256 of the opaque token; the raw value only ever exists in the mail
  "expiresAt" timestamp NOT NULL, "usedAt" timestamp, "revokedAt" timestamp,  -- a new request revokes the previous unused link
...
analytics_events (                                               -- first-party product analytics (ADR-86, ALPHA_PLAN 7.5)
  id uuid PK, "profileId" uuid NULL REFERENCES profiles ON DELETE SET NULL,
  name varchar(64),                                              -- closed union, enforced in the application (AnalyticsEventName)
  "occurredAt" timestamp, "createdAt" timestamp DEFAULT now(),
  properties jsonb DEFAULT '{}',                                 -- numbers/booleans/tags <= 32 chars only; the service strips the rest
  INDEX (name, "occurredAt"), INDEX ("profileId", "occurredAt")
)
...
title_posters (                                                  -- POSTERS-MULTI P1, ADR-120; schema only, empty until P2
  id uuid PK, "titleId" uuid NOT NULL FK titles ON DELETE CASCADE,
  "posterPath" varchar NOT NULL,                                 -- TMDB path only ("/abc.jpg"); CHECK ~ '^/[A-Za-z0-9_-]+[.][A-Za-z0-9]+$' refuses a composed URL (ADR-82 enforced, not just documented)
  "sortOrder" integer NOT NULL DEFAULT 0,                        -- CHECK >= 0; ascending display order, tie-broken by "posterPath"; 0 is the image titles."posterPath" already carries
  "sourceRecordId" uuid FK source_records ON DELETE NO ACTION,   -- the rights row for this image; a registry row is superseded, never deleted
  "createdAt" timestamp NOT NULL DEFAULT now(),
...
shared_latent_space_versions (                                   -- BP §7.5
  ...
  "trainingCohortSize" integer, "acceptanceGateMetrics" json, active boolean NOT NULL DEFAULT false, "createdAt" timestamp NOT NULL DEFAULT now()  -- NOT NULL/DEFAULT: AUDIT_2026-09-05 M4
)
```
...
| M6 ✅ | `public_quality_sources`, `availability_snapshots` — all applied by `AddM6PublicQualityAndAvailability` | Public Quality and Watchability layers — `public_quality_sources` populated since ADR-77 (IMDb); `availability_snapshots` still needs a licensed availability partner |
...
- 2.30 (2026-09-05): twenty-sixth migration `AddTitleIdIndexes` -- `IDX_embeddings_titleId`, `IDX_recommendations_titleId`, `IDX_watch_events_titleId` (AUDIT_2026-09-05 H7), additive; applied to `movie-postgres` and `moviedb_test`.
- 2.29 (2026-09-04): twenty-fifth migration `AddAnalyticsEvents` -- `analytics_events` (ADR-86, ALPHA_PLAN 7.5). `profileId` is `ON DELETE SET NULL` like `consents`/`privacy_requests` (ADR-80) so deleting a profile does not rewrite the funnel counts; `name` is a closed union enforced in the application, `properties` jsonb keeps only numbers/booleans/short tags; indexes on `(name, occurredAt)` and `(profileId, occurredAt)`. Rows are written only for a profile whose `analytics_first_party` consent is granted.
- 2.28 (2026-09-04): twenty-fourth migration `AddPasswordResets` -- `password_resets` (ADR-85, ALPHA_PLAN 3.2). Hash-only like `refresh_tokens`, cascading with the account; single use via `usedAt`, and a new request revokes the previous unused row.
- 2.27 (2026-09-04): twenty-third migration `AddTitlePosterPath` -- `titles.posterPath` (ADR-82, owner decision on posters). Path only; `PosterService` composes the URL per read and withholds it unless the image's `source_records` row is displayable at this stage. NULL on `movie-postgres` until `seed-demo.lib.ts` (session C's) maps the fixture's `posterPath` the way it already maps `originalLanguage`.
- 2.26 (2026-09-04): twenty-second migration `ConsentsTombstone` applied -- `consents.userId` nullable with `ON DELETE SET NULL` plus an indexed `subjectKey` (ADR-80, board F5). Consents now survive a purge as a personal-data-free record, the treatment `privacy_requests` got in `PrivacyRequestsTombstone`; rows predating this migration keep `subjectKey` NULL (no backfill, same as the sibling). `POST /privacy/{pause,resume}` write `profiles.pausedAt`, which recommendations now refuse to serve against.
