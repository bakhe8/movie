...
- **§1 Current physical schema** — exactly what the sixteen TypeORM migrations in `apps/backend/src/migrations/` create (verified 2026-09-03). This is the truth for anyone writing SQL today.
...
user_model_snapshots (                                           -- one row per training run (BP §13.1 taste_profiles, partial)
  ...
  posterior json,                                                -- per-weight uncertainty (BP §13.1); never populated -- fit() doesn't compute one
...
| M5 ✅ | `recommendations`, `outcomes`, `watch_events`, `library_imports` — all applied by `AddM5RecommendationsAndWatchEvents` | persisted recommendations, post-watch loop, imports — schema step only; `recommendations` itself is now written (ADR-58), `outcomes`/`watch_events`/`library_imports` still aren't |
