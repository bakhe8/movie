...
| GET | `/api/titles` | — | `?query&page&limit(≤100)&genre&year(1870-2100)` | `{ items, page, limit, total, totalPages }` | ILIKE on `titleEn`/`titleAr`; Arabic folding on both sides (hamza forms of alef → ا, ة → ه, ى → ي, tashkeel/shadda/dagger alef/tatweel stripped from query **and** column) so «احلام» finds «أحلام» and «الرسالة» finds «الرِّسالة»; `unaccent` on `titleEn` so «Amelie» finds «Amélie»; alternate titles matched through `localized_titles` with the same folding (ADR-106; nothing populates that table yet). `genre` and `year` (added 2026-09-06) filter server-side across the whole catalogue, not just the returned page — `genre` matches one of `Title.genres` exactly (never a substring), `year` matches `releaseYear` exactly; both combine with `query` and each other with AND. Frontend: `DiscoverScreen`'s genre chips now query this instead of filtering the client's already-loaded results |
...
| POST | `/api/profiles/:profileId/train` | JWT | — | `{ jobId, status, created }` (202) | compatibility/operator-recovery endpoint, not a first-party user interaction (ADR-114). It enqueues a durable `training_jobs` row (ADR-100) and dispatches at once; `created: false` when an identical non-terminal request exists. 400 `{ reason: 'need_more_triads', needed: 1 }`; 409 `{ reason: 'paused' }`; 503 `{ reason: 'model_service_disabled' }`. Normal training is automatic after the 3rd completed learning triad and every 5th after that; readiness/recommendation/library reads also idempotently repair an eligible-idle or schema-stale model. The UI must never require this route to reach a result |
...
| GET/PATCH/POST | `/api/admin/**` | JWT + role `admin` | see below | see below | the internal board (`BP §5.1`, SPECIFICATION §5.5). `AdminGuard` after the JWT guard: 401 anonymous, 403 `{ reason: 'admin_required' }` for a signed-in non-admin. Every write leaves an `audit_log` row naming the acting admin. Routes: `GET context` (ADMIN-W1, ADR-117: `{ user: { id, email, role }, capabilities }` — the access-boundary probe the frontend gates every destination on instead of a throwaway read; `capabilities` come from `capabilitiesForRole()`, currently the legacy `admin` role mapped to every entry in `ADMIN_CAPABILITIES`, and are a UI hint only — the server, not the client, still enforces every operation) · `GET titles?query&missing=fingerprint\|v2\|license&page&limit` (rows with `hasFingerprint`, `hasV2`, worst `licenseStatus` across `source_records`, `unreviewedFeatures`) · `GET titles/missing-fingerprints` · `GET titles/:id` (full row incl. `fingerprint`/`externalIds`) · `GET titles/:id/provenance` (rights rows + every feature row, current and superseded, grouped by extractor) · `PATCH titles/:id` (source data only: `titleEn`, `titleAr`, `description`, `releaseYear`, `genres`, `originalLanguage`, `externalIds`; never the fingerprint; write + audit share one transaction, ADMIN-W4 ADM-P0-03) · `POST titles/:id/source-records` (rights registry, `BP §11.1`; transactional) and `PATCH source-records/:id` (ADMIN-W4 ADM-P0-04: never edits in place — creates a new row with the patched fields and points the old row's `supersededBy` at it, transactionally; **409** `{ reason: 'already_superseded', currentRecordId }` when the target row already has a successor) · `GET content-features?reviewStatus&titleId&featureKey&extractorVersion` (review queue, current rows only) · `GET content-features/sample?size&extractorVersion` (`BP §15.4` random sample) · `POST content-features/:id/review { reviewStatus, correctedValue?, note? }` → `{ feature, correction, republish }` (a corrected value becomes a new `human-review-v1` row that supersedes the extracted one, returned as `correction`; `feature` is the original row, now carrying `supersededBy` when a correction was created; originals are never edited — a caller that only reads `id`/`reviewStatus` from a flat body has the wrong shape, ADMIN-W1. ADMIN-W4 ADM-P0-02: `republish` is `null` when there was no correction or the title has no published fingerprint yet, else `{ changes: { featureKey, before, after }[] }` — the correction's effect on `titles.fingerprint` folded in atomically, in the same transaction, so the trainer/API see it immediately instead of waiting on the standalone `republish-fingerprint` script) · `GET models` → `{ versions: (ModelVersion & { stats: SnapshotStats | null })[], unregistered: SnapshotStats[] }` (registered `model_versions` beside `user_model_snapshots` stats per version, plus versions seen in snapshots but never registered; `SnapshotStats = { modelVersion, snapshotCount, profileCount, latestAt, meanHeldOutPairwiseAccuracy, meanHeldOutNll }` — these are the wire names since ADMIN-W1 (ADR-117 ADM-P0-01); the query builder's internal aliases `snapshots`/`profiles` never leave the service) · `POST models` · `PATCH models/:version { active?, evalReport?, thresholds? }` (exactly one active version; activating another is the rollback of `BP §18.1`, and serving honours it since ADR-76) · `GET experiments` (with arm counts) · `GET triads/latest?limit` (pseudonymous profile ids only) · `GET users?query` · `PATCH users/:id { active?, role?, reason? }` (an admin cannot change themself, 403 `self_change`; ADMIN-W4: deactivating or demoting the last other active admin is refused too, 403 `last_admin` — defence-in-depth, since a live actor can never legitimately zero out active admins on their own; deactivation revokes every live refresh token) · `GET privacy-requests?type&status` · `GET audit-log?actorUserId&action&resource&resourceId` · `GET mail-outbox` (the mail outbox, ADR-97: counts per status and the last 20 rows with kind, status, attempts, next attempt, last error and provider id — never the address or the body) · `GET training-jobs?limit` (ADR-100: the durable training queue, same shape as `mail-outbox` — counts per status and the recent rows with attempts, `errorKind`, sanitized `lastError`, next attempt, `updatedAt` (the row's own last-write time — never derive "last updated" from `finishedAt ?? createdAt`, ADMIN-W1 ADM-P1-07), pseudonymous `profileId` only) · `GET readiness` (ADR-100: `{ database: { ok }, catalog: { titles, threshold, ok }, fingerprintCoverage: { published, total, percent, ok }, modelService: { configured, reachable, ok } }` — whether training can plausibly succeed right now, not any one profile's own eligibility). The first admin is granted on the server: `npm run admin:grant -- <email>` (audited as `operator`) |
...
| GET | `/api/profiles/:profileId/library/ranking` | JWT | — | LibraryRankingItem[] | the profile's watched, fingerprinted titles ordered by the latest snapshot (`BP §5.3` "ترتيب شخصي"); positions only, the score never leaves the server (ADR-33); same band/demotion rules as recommendations; 409 `{ reason: 'model_pending'\|'model_refreshing' }` until derived state is ready, after idempotently scheduling any eligible build/refresh (ADR-114); first-party UI polls it automatically; `[]` when nothing is watched |
| GET | `/api/profiles/:profileId/recommendations` | JWT | `?limit(≤50, default 10)` | `{ state: 'ready', items: Recommendation[] }` \| `{ state: 'pending', needed, training }` \| `{ state: 'paused' }` | always **200** for a profile the caller owns (ADR-81/114). `pending` carries rounds still needed plus durable training state; once enough evidence exists, the read idempotently schedules an eligible-idle build, and a schema-stale snapshot is scheduled then returned as ordinary `pending` so the UI's silent poll follows it. `paused` remains explicit because the person withdrew processing authority. Excludes watched titles only; unknown fingerprint dimensions use pool-mean imputation; writes recommendation rows with `shownAt: null` until impressions are reported; each item carries `recommendationId`, separate Public Quality/availability fields, and no market/platform input to Personal Fit |
...
| GET | `/api/profiles/:profileId/readiness` | JWT | — | `{ rounds, ordinalModel, semanticProfile, recommendation, availability }` | ADR-103/114: four capabilities, each `{ status, reason, action, publishedAt, modelVersion, confidenceBand }`. `action` names only an actual decision available to the person: `mark_watched_titles`, `rank_more_triads`, `resume_processing`, or `null`. Eligible/queued/processing/failed/stale, model-service, fingerprint-coverage, catalogue-candidate and availability-source states always return `null`; the system/operator owns them. An eligible-idle or schema-stale read schedules training idempotently before returning. `rounds` remains `{ learningRounds, verificationRounds, firstTrainingAt, nextTrainingAt, watchedTitles, suggestedWatchedTitles }`; fewer than three eligible watched titles yields `mark_watched_titles`, otherwise insufficient evidence yields `rank_more_triads` |
...
| POST | `/api/v1/triads/:id/rank` | save one complete ranking | checks membership, `answeredAt` window, not already answered; requires `Idempotency-Key`; stores `answeredAt`, `modelVersion`; confirms all three titles `watched` and logs a `watch_events` row per title (`source: 'triad_ranked'`, ADR-119) -- exposure bookkeeping only, `ranking` remains the one preference signal | `/api/triads/:id/rank` |
...
| POST | `/api/v1/watch-events` | record a watch (and edition) | body `{ profileId, titleId, watchedAt?, source: 'in_app'\|'import'\|'manual', audioLanguage?, subtitleLanguage?, provider? }`; does not imply liking; if the title was recommended, links an `outcomes` row. `source` also has an internal-only `'triad_ranked'` value (ADR-119) this endpoint's own `@IsIn` never accepts -- only `POST …/rank` writes it | `POST /api/profiles/:profileId/watch-events` (implemented 2026-09-03, ADR-66; `profileId` in the URL not the body, matching every other route this session, not the target's exact shape) |
...
**Changelog**
- 1.14 (2026-09-06): `GET /api/titles`/`search` gain `genre` and `year` query params (feasibility report follow-up) -- filter the whole catalogue server-side; `DiscoverScreen`'s genre chips moved from filtering the client's loaded page to this.
