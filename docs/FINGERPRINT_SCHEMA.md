...
- **Every model call is accounted for** (board request G7, owner's decision 2026-09-04, [DATA_LICENSING.md](DATA_LICENSING.md) §0): the worker records, per call, the purpose (`fingerprint_v1|fingerprint_v2|fingerprint_v3|explanation`), the model requested and the model served, input/output/cache tokens as the API reported them, latency, status (`ok|refused|ceiling|unparsed|error`), the title's id and the extractor version — one JSON line per call in `$LLM_CALL_LOG_DIR/<runId>.jsonl` (outside git) and a summary table per purpose × served model at the end of every run report. Cost is never stored as a fact: `costUsd` is filled only when `LLM_PRICES_JSON` names prices for the served model, otherwise null, so the profitability study computes it from the tokens with the prices of its day. No prompt or evidence text is ever logged.
...
- **Second plot in Arabic** (2026-09-04, plan §7.5): `fetch-evidence-ar.ts` stores the Arabic Wikipedia plot section as `evidence.plotSummaryAr` (same section rule as the English plot, `wiki-plot.lib.ts`); when the English plot is under 2,000 characters the runner appends it as a second account of the same film, adds the article to `sourceIds` and stamps `enrichment-worker-v2+ar-evidence` — a different evidence policy is a different version, so the seed supersedes the earlier rows.
