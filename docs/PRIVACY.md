...
**One exception, stated because it is true (P1-1):** poster images are loaded by the browser directly from `image.tmdb.org`. There is no same-origin proxy, so TMDB receives the viewer's IP address and user-agent for every poster shown, exactly as any embedded third-party image would. Two things bound it: the poster element carries `referrerPolicy="no-referrer"`, so no page address travels with the image; and a poster URL carries no identifier of ours. The work page also paints the same image as a blurred CSS backdrop, which no per-element attribute can reach — there the browser's own default (`strict-origin-when-cross-origin`) sends our origin and never the path, so the film being viewed is still not disclosed. A document-wide `Referrer-Policy` header would close even that, and is deliberately not set: the owner's constraint on this change is that poster loading must not be altered beyond the image attribute itself (2026-09-05). A proxy would end even the IP exposure, and is deliberately **not** built yet: proxying would make us the redistributor of TMDB's images, which is an open licensing question (ADR-72, DATA_LICENSING §4), not a technical one. Revisit when that is settled.
