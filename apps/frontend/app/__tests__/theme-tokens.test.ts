...
// Normalised on read: the baseline hash below is over the file's content, not
// over whichever line ending a checkout happens to use. Without this the dark
// guard passes on Linux CI and fails on every Windows working copy, which is
// a false alarm about the one thing it exists to protect.
const css = readFileSync('app/styles/tokens.css', 'utf8').split('\r\n').join('\n');
