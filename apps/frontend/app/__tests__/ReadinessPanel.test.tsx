...
describe('ReadinessPanel (ADR-103 on screen)', () => {
  ...
  it('tells the two "no suggestions" cases apart: no model, versus no candidates', async () => {
    ...
    await waitFor(() => expect(screen.getByText(/لا توجد عناوين جديدة مؤهلة/)).toBeInTheDocument());
  ...
  it('distinguishes marking watched titles from ranking once a triad is possible', async () => {
    mockApi.getReadiness.mockResolvedValue(
      readiness({ ordinalModel: capability({ status: 'not_ready', reason: 'insufficient_triads', action: 'mark_watched_titles' }) }),
    );
    render(<ReadinessPanel profileId="p1" lang="ar" />);
    await waitFor(() => expect(screen.getByText(/سجّل أفلامًا شاهدتها/)).toBeInTheDocument());
  });
  ...
  it('re-reads system-owned progress automatically without a refresh control', async () => {
    vi.useFakeTimers();
    const automatic = readiness({
      ordinalModel: capability({ status: 'eligible', action: null }),
      semanticProfile: capability({ status: 'eligible', action: null }),
      recommendation: capability({ status: 'eligible', action: null }),
    });
    mockApi.getReadiness.mockResolvedValue(automatic);
    render(<ReadinessPanel profileId="p1" lang="ar" />);
    await act(async () => { await Promise.resolve(); });
    expect(mockApi.getReadiness).toHaveBeenCalledTimes(1);
    ...
    expect(mockApi.getReadiness).toHaveBeenCalledTimes(2);
  });
});
